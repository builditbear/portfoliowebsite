...
function animateConsoleText(words, text_element_id, color = "#FFF5EE") {
  ...
  let terminal_cursor = document.querySelector("#terminal-cursor");
  ...
  window.setInterval(() => {
    if (terminal_cursor_visible === true) {
      terminal_cursor.className = "text-cursor hidden-by-opacity";
      terminal_cursor_visible = false;
    } else {
      terminal_cursor.className = "text-cursor";
